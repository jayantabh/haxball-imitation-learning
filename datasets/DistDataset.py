...
class DistDataset(Dataset):
    ...
    def filter_states(self):
        ...
        for state, player_team in self.game_states:
            ...
            if sum(our_player.input) == 1:
                processed_states.append((state, player_team))

            # if sum(our_player.input) == 2 and random.random() < 0.5:
            if sum(our_player.input) == 2:
                processed_states.append((state, player_team))
    ...
    def __getitem__(self, idx):
        ...
        # Add distance to each player
        sample['input'].append(math.dist((our_player.disc.x, our_player.disc.y), (state.ball.x, state.ball.y)))
        sample['input'].append(math.dist((opp_player.disc.x, opp_player.disc.y), (state.ball.x, state.ball.y)))
